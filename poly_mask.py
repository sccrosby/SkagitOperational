...
class Polygon:
    ...
    def is_inside(self, xpoint, ypoint, smalld=1e-12):
        ...
        if xpoint.shape != ypoint.shape:
            raise IndexError('x and y have different shapes')
...
if __name__ == '__main__':
    ...
    # Test
    x = np.linspace(525947.212, 548152.836, 100)
    y = np.linspace(5343440.267, 5366725.231, 100)

    #xx, yy = np.meshgrid(x, y)
    grid = poly.is_inside(x, y)
    # Plot
    import matplotlib.pyplot as plt
    plt.plot(xvert, yvert, 'k-')
    plt.contour(xx, yy, grid, levels=np.linspace(-100, 100, 51), colors='k')
    plt.pcolor(xx, yy, grid, vmin=-2500, vmax=2500, cmap=plt.get_cmap('RdBu'))
    plt.colorbar()
